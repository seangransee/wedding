...
- `wedding_rsvp_attendees` - place-card names for a current RSVP, keyed by RSVP and 1-based `position`.
...
- The server currently stores only non-empty attendee names. It does not enforce that every attending slot has a place-card name, even though the UI asks guests to enter names.
